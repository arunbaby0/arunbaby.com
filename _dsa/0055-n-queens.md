...
- On a 4x4 board, there are `C(16, 4) = 1,820` ways to place 4 queens.
- On an 8x8 board, there are `C(64, 8) = 4,426,165,368` ways.
- On a 16x16 board, the number is approximately `3.6 x 10^20`.
...
The shared theme across all four tracks is **Constraint Satisfaction and Search**:
...
- We only search for the first queen in columns `[0, floor(N/2)]`.
- If `N` is odd and we place the first queen in the exact middle column, we only need to search the second queen in columns `[0, floor(N/2)]` for that branch.
...
- **Variables (V):** `{Q_1, Q_2, ..., Q_n}`, where `Q_i` is the column of the queen in row `i`.
- **Domains (D):** `{1, 2, ..., n}` for each variable.
- **Constraints (C):** 
  - `Q_i != Q_j` (Cols must be different)
  - `|Q_i - Q_j| != |i - j|` (Diagonals must be different)
...
**A:** Instead of an `N x N` board, use an array of size `N` where `arr[i] = j` means a queen is at `(i, j)`. This reduces space from O(N^2) to O(N). For the "Is Safe" check, use bitsets or the `r-c/r+c` arrays we discussed.
...
- **Nodes**: Every square `(i, j)` on the `N x N` board is a node. There are `N^2` nodes.
