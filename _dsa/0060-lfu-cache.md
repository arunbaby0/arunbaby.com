...
Today, on Day 60, we tackle the **LFU Cache**, one of the most challenging data structures to implement with $O(1)$ efficiency. We will explore why it is hard, how to solve it with a linked-list-of-linked-lists, and how it connects to the future of persistent intelligence in AI.
...
class LFUCache:
    ...
    def _update_node(self, node):
        ...
        # If the min_freq list is empty, we know the new min_freq must be old_freq + 1
        # unless a new key is added (which resets min_freq to 1)
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if key in self.key_to_node:
            ...
            if self.size >= self.capacity:
                ...
                # The pop_tail() returns the LRU node within the LFU tier
...
### Hidden Constants
While both LRU and Tiered-DLL LFU are $O(1)$, the **Hidden Constant** for LFU is significantly higher. You are managing multiple hash map lookups (`key_to_node` and `freq_to_list`) for every single access. In a high-traffic system, LFU can be 2-3x slower than LRU in terms of raw CPU cycles.
...
LFU has a fatal flaw: its memory is "sticky." If a key was very popular in the morning (e.g., "Trending Morning News") but is never used again, it will stay in the cache forever because its frequency is too high to be evicted by new, more relevant items.
...
- It dynamically resizes the "target size" for each list based on which list is currently yielding more "hits."
- If recency hits are high, it allocates more memory to the LRU tier.
- This creates a self-tuning system that adapts to the workload.
...
Databases like SQLite or MySQL use LFU-like logic to keep the "Indices" (accessed on every query) in memory while allowing the "Data Pages" (accessed once per scan) to be evicted quickly.

### 9.2 Machine Learning Feature Stores
In the **ML System Design** world, we use LFU for **Feature Caching**.
- A feature like `user_timezone` is used in every prediction. It should be cached.
- A feature like `last_five_actions` changes every second. 
- LFU ensures that globally popular features (like `is_weekend`) stay in the fastest cache layer.

### 9.3 Speech Technology
In **Speech Tech** (Day 60 theme), we use LFU to cache **Phonetic Embeddings**. Common words like "The," "Is," and "Hello" have their acoustic-to-text mappings cached in an LFU tier to save GPU inference time.
...
2.  **Propose the Tiered Map**: Draw a diagram showing `freq -> DLL`. This is the clearest way to explain $O(1)$.
3.  **Explain the Eviction Tie-break**: Mention that you use LRU *within* each frequency tier. This shows you understand the constraints.
4.  **Discuss min_freq**: Explain how you maintain $O(1)$ eviction by tracking the global minimum frequency without a loop.
...
3.  **Future-Proofing**: As AI agents move toward long-term memory (persistently storing what is important), the algorithms of LFU will become the foundation of "Thinking Architectures."
