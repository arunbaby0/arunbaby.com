...
In Day 21, we discussed the "Composability" approach to vision. We built a stack: `Image -> GroundingDINO -> Bounding Box -> SAM -> Mask -> Classifier -> Label`. While powerful for real-world objects like cars or cats, this approach struggles with the abstract, high-density information of a **Screenshot**.
...
In our ML series (Day 22), we discuss **Cost Optimization**. Vision Agents are the most "Token-Expensive" agents in existence. A single screenshot can cost 100x more than a sentence. Mastering image tiling (Section 2.2) and visual summarization (Section 6) is fundamentally an exercise in **ML Cost Engineering**.

Similarly, in DSA (Day 22) we look at **Minimum Path Sum**. When an agent is navigating a screenshot (Day 23), it is trying to find the "Minimum Path" of clicks to reach a goal. Every unnecessary click is a "Cost" (both in time and tokens).
...
This knowledge bridges the gap between raw pixels and smart actions. In the next post, we will apply these MLLMs to a specific domain: **Screen Agents**, building our first "Computer-Using Agent" that can literally interact with any software ever written.
